...
impl DigitalFrontend {
    ...
    pub fn process_block(&mut self, raw_floats: &mut [f32]) {
        ...
        for chunk in block_planar.chunks_exact_mut(16) {
            let a = f32x8::from_slice(&chunk[0..8]);
            let b = f32x8::from_slice(&chunk[8..16]);

            let (mut re_v, im_v) = a.deinterleave(b);  // Now i_v and q_v contain the I and Q components of 8 samples separately
            ...
            for i in 0..8 {
                ...
                self.nco.phase_accumulator =
                    (self.nco.phase_accumulator + self.nco.phase_step) % self.nco.lut_size;
            }
